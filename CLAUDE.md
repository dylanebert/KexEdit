...
- `kex2d/` — 2D coaster solver prototype (Shallot + Svelte + canvas2D). Free-drag nodes → stored-heading cubic Hermite → physical F_n, being re-architected to a position-space collocation solver. Parallel to `app/`. Model + code map: `kex2d/CLAUDE.md` (auto-loads in-tree). Priority + design rationale, decision record, live status: `kex/specs/kex2d-collocation.md`
