...
- **A value axis displays.** Auto-fit freely at rest — the y g-range fits like the AE/Unity curve editors; a live gesture keeps its displayed axis fixed while its fitted target stays live.
...
- **Set + active member** (Blender's active object; Unity's `activeGameObject`): one container of
  typed `{kind, id}` members with the last-selected member active; single-select is the size-1
  case — one substrate, never a parallel multi path beside scalar selections. Cross-kind
  co-selection is ordinary (shift-click and marquee extend across kinds); replace-select clears
  sibling kinds but keeps an ancestor kind of the clicked member (containment, not siloing — a
...
vocabulary, below). Hover's boundaries travel with the rung: a foreign gesture suppresses hover,
while the surface under its own gesture keeps its read. Hover is invisible on an already-selected
element (selection is the stronger read of the same span), while a distinct resize handle keeps its
hover over the selected span; kex2d `.clip-trim` is the in-project exemplar. There is no
cross-surface hover sync — a clip's CSS hover and the viewport span stay local to their own surfaces.
...
- **Author in the solver's invariant domain.** The axis a constraint is placed on must be one the
  ...
  rejection); authoring on an axis requires the store to hold that axis's unit fixed — kex2d
  earns it by storing every force keyframe, extent, strip and strip keyframe in arclength always
  and projecting a Time reading through the live bake's s↔t table, frozen per gesture, rather than
  converting the store itself.
