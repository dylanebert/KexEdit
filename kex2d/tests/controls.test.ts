...
describe("sectionKeyAct — the whole-section Delete rungs", () => {
    ...
    test("`D` fires solve when canSolve is true", () => {
        expect(sectionKeyAct("d", { opsAllowed: true, multi: false, canSolve: true })).toBe(
            "solve",
        );
        expect(
            sectionKeyAct("D", {
    ...
    test("`P` fires pinEnter only when canPin is true", () => {
        expect(sectionKeyAct("p", { opsAllowed: true, multi: false, canPin: true })).toBe(
            "pinEnter",
        );
        expect(sectionKeyAct("P", { opsAllowed: true, multi: false, canPin: false })).toBeNull();
    });
    test("`R` fires reset only when canReset is true", () => {
        expect(sectionKeyAct("r", { opsAllowed: true, multi: false, canReset: true })).toBe(
            "reset",
        );
        expect(
            sectionKeyAct("R", {
