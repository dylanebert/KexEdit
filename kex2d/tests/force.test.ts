...
describe("forces64 — geometry→force accuracy", () => {
    test("matches the analytic circle force to roundoff at any spacing (Menger exactness)", () => {
        // a constant-curvature arc (κ=1/R) sampled uniformly in arclength: an
        // exact continuum reference. Menger κ (circumcircle of 3 points) is
        // EXACTLY 1/R for any 3 points on the circle, and the neighbor-chord
        // tangent is exact under symmetric spacing — so the chord-based map is
        // exact here at every ds, limited only by roundoff. the error floor
        // scales like eps/ds² (cancellation in the cross product of O(ds)
        // vectors), so the derived bound is 100·eps·(v0²/g)/ds². R,v0 chosen
        ...
        for (const ds of [0.5, 0.25, 0.125]) {
            expect(err(ds)).toBeLessThan((100 * eps * ((v0 * v0) / G)) / (ds * ds));
        }
    });
    ...
    test("matches the integrator source force to O(ds), bounded by ½·ds·max|κ'|·v²/g", () => {
        ...
        const measure = (ds: number) => {
            ...
            const { fN: F64, kappa } = forces64(x, y, N, v0);
    ...
    test("position round-trip forward(forces64(P°)) stays within the conditioning envelope", () => {
        ...
        const { fN: F64 } = forces64(p.x, p.y, N, v0);
...
describe("forceJacobian — analytic ∂F/∂P vs central finite differences", () => {
    ...
    function fd(i: number, j: number, axis: 0 | 1): number {
        ...
        const base = arr[j];
        arr[j] = base + h;
        const fp = forces64(x, y, N, v0).fN[i];
        arr[j] = base - h;
        const fm = forces64(x, y, N, v0).fN[i];
        arr[j] = base;
        return (fp - fm) / (2 * h);
    }

    test("the six in-band entries match FD entrywise", () => {
        const J: ForceJac = forceJacobian(x, y, N, v0);
...
describe("conditioning thesis — geometry→force is banded / flat in N", () => {
    ...
    test("σ_max(∂F/∂P) is flat in N (log-log slope < 0.2)", () => {
        ...
        const sig = Ns.map((N) => {
            const F = washboard(N, ds);
            const { x, y } = rollout(F, N, ds, v0);
            return sigmaMax(forceJacobian(x, y, N, v0));
        });
