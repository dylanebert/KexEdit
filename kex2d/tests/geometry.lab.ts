...
console.log("    ds     vs continuum (O ds²)    ratio     vs integrator (O ds)   ratio");
{
    ...
    for (const ds of [0.5, 0.25, 0.125, 0.0625]) {
        ...
        const fc = forces64(x, y, Nc, ds, v0c).fN;
        let ec = 0;
        ...
        const fi = forces64(p.x, p.y, Ni, ds, v0i).fN;
        let ei = 0;
...
// ── panel 2: the banded ∂F/∂P sparsity map ──
{
    ...
    const J = forceJacobian(p.x, p.y, N, ds, v0);
    const cols = 2 * N;
...
// ── panel 3: the conditioning thesis ──
{
    ...
    for (const N of Ns) {
        const p = positions(rollout(washboard(N, ds), N, ds, v0));
        const sFP = sigmaFP(forceJacobian(p.x, p.y, N, ds, v0));
        const sPF = sigmaPF(N);
        fp.push(sFP);
        pf.push(sPF);
        console.log(
