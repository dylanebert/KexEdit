...
- `solve.ts` — the convex F_n optimizer (pure, no ECS). `solve(Fpos, opts)` minimizes data L2 + second-difference smoothness + a soft force-limit band hinge over the solved F_n; a banded pentadiagonal LDLᵀ wrapped in an active-set loop for the hinge (warm-startable). `DEFAULT_BAND` `[−2, 6]g`, `DEFAULT_SMOOTH`/`DEFAULT_BAND_WEIGHT` (live feel weights, tune by eye). Roadmap "kex2d Phase 1".
