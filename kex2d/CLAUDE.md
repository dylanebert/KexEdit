...
- `cart.ts` — looping cart animation on the *realized* (solved) track. `cartState[trackEid].t` advances by real `dt`, wrapping at `loopTime(solveOut)` — the realized track's first infeasible time, or its `tTotal`. `cartPose(trackEid, t)` interps `solveOut` for the box renderer and returns `u` (draft-time grid-fraction progress for the strip cursor); `sampleFNOverTime` resamples the *baked* F_n onto a uniform-time grid (the strip's draft dots).
