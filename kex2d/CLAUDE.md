...
- `solve.ts` — the convex F_n optimizer (pure, no ECS). `solve(Fpos, opts)` minimizes data L2 + second-difference smoothness + a soft force-limit band hinge over the solved F_n; a banded pentadiagonal LDLᵀ (`pentaFactor` + `pentaSolveFactored`) wrapped in an active-set loop for the hinge (warm-startable). `DEFAULT_BAND` `[−2, 6]g`, `DEFAULT_SMOOTH`/`DEFAULT_BAND_WEIGHT` (live feel weights, tune by eye). Phase 2a adds `con` (point force pins — constant diagonal data terms) and `anchors` (recovery rows from `anchor.ts` — dense, entering via a **Woodbury** low-rank update around the band; the bias `U W t`, `t=row·Fpos`, pulls geometry to the *draft*, not 0; the active-set test reads the post-Woodbury solution). Roadmap "kex2d Phase 1–2".
...
- `timeline.ts` — pure transform + tick math for the force-curve timeline (no Svelte/DOM/solve state). `View` = `{pan, pxPerSec}`; `secToPx`/`pxToSec` (the second↔pixel affine), `zoomAt` (cursor-anchored geometric zoom — the second under the cursor stays fixed; clamp scale *before* deriving pan), `clampView` (pan + scale clamped to the track extent + margin; min-scale fits the whole track and centers it), `frameAll`, `niceStep` (1-2-5×10ⁿ, geometric-mean breakpoints), `ticks`. Ported from `reference/animation-timeline`; unit-tested in `tests/timeline.test.ts`.
- `Timeline.svelte` — the always-present bottom dock that replaced the old static F_n strip. A seconds X-axis (draft-time, zero-anchored) with adaptive 1-2-5 ticks, the baked F_n dots + solved curve on a canvas2D dense layer, a scrubable SVG playhead tracking the cart, and a persistent centered play/pause transport (also Space) with an always-on time readout. The Y range tracks the solve's force band (`DEFAULT_BAND` + 1g headroom), drawn with band-limit + 1g lines, so a curve never clips. View-state is one `{pan, pxPerSec}` in `$state`; a `clamped` `$derived` re-fits it to the live width/track-length (so a resize or edit never writes back into `view` — no effect loop). Wheel zoom on ctrl/⌘ (geometric, cursor-anchored) else pan, both clamped to the track extent — so zooming fully out *is* frame-all (no separate button). Opaque surface, **no resize** — follows shallot's editor posture (`shallot/.claude/rules/editor-ui.md`: a docked surface is opaque + fixed, transport persistent). X-axis = draft-time grid seconds; the cart clock is realized time, bridged by `u` (consistent only because the axis samples at `OPT_GRID`).
