...
test("section menu + keyframe flow", async ({ page, boot }) => {
    ...
    // row, fitted to this section's kind (Convert to geo, live), the domain flip (force sections
    // only), and Delete. The opposite conversion direction is absent, not grayed: a section is
    // one kind, so the other row could never fire.
    ...
    await expect(page.locator(".ctxmenu").getByRole("menuitem", { name: "To time" })).toBeEnabled();
    await expect(
...
// Drive the TIME-DOMAIN force section end to end (kex2d-time-domain stage 4): append one
// through the real flyout's third row → place a keyframe on it by cursor position → prove a
// grab-and-return writes NOTHING (the delta-from-grab law: an absolute re-derivation through
// the display inverse would bake the lens's O(step) gap into authored state) → drag it for
// real → change v₀ upstream and prove the point's DRAWN d moved while its STORED t held (the
// invariant-domain generalization: a time keyframe rides its authored t and its distance is
// derived) → flip the domain through the real context menu, and undo it byte-identically.
test("time-domain force section flow", async ({ page, boot }) => {
    await boot();

    const domains = () => page.evaluate((): number[] => (window as any).__kex.sectionDomains());
    const pts = (i: number) =>
        page.evaluate(
            (k: number): { id: number; s: number; g: number; d: number }[] =>
                (window as any).__kex.forcePointsAt(k),
            i,
        );
    ...
    // ── 1. Append a TIME force section through the real `+` flyout — the new third row. ──
    await seedHill(page);
    await page.locator(".clip-add").click();
    await page.getByRole("menuitem", { name: "Append time-domain force section" }).click();
    await expect.poll(async () => (await domains()).join(",")).toBe("0,1");
    await frameTimeline(page);
    // the clip label carries the NATIVE extent: the axis under it is metres, so a time
    // section's authored duration is otherwise unreadable from its width.
    await expect(page.locator(".clip-label").nth(1)).toContainText("s");
    await page.waitForTimeout(SHOT_MS);
    if (strip) await page.screenshot({ path: join(OUT, "time-1-append.png"), clip: strip });

    // ── 2. Place TWO keyframes on it by cursor position (the chart's own insert path, now
    // resolving the section-local coordinate through the lens instead of `d − offset`). Two,
    // because the gestures below must land on DIFFERENT diamonds: two presses on the same one
    // inside `FDBL_MS` are the handle-edit summon, not a second drag. ──
    const body = page.locator(".dock .body");
    const bb = await body.boundingBox();
    const fcb = await page.locator(".clip").nth(1).boundingBox();
    if (!bb || !fcb) throw new Error("timeline body / time clip not laid out");
    const seeded = await pts(1);
    await page.mouse.dblclick(fcb.x + fcb.width * 0.3, bb.y + bb.height * 0.5);
    await expect.poll(async () => (await pts(1)).length).toBe(seeded.length + 1);
    await page.mouse.dblclick(fcb.x + fcb.width * 0.65, bb.y + bb.height * 0.5);
    await expect.poll(async () => (await pts(1)).length).toBe(seeded.length + 2);
    const fresh = (await pts(1)).filter((p) => !seeded.some((q) => q.id === p.id));
    if (fresh.length !== 2) throw new Error("the inserted keyframes weren't found by id-diff");
    const [held, moved] = fresh;
    // they landed in SECONDS, inside the section's own duration (~2.4 s), not in metres.
    const dur = (await lengths())[1];
    for (const p of fresh) {
        expect(p.s).toBeGreaterThan(0);
        expect(p.s).toBeLessThan(dur + 1e-6);
        expect(p.d).toBeGreaterThan(p.s); // its drawn distance is metres, far larger
    }
    await page.waitForTimeout(SHOT_MS);
    if (strip) await page.screenshot({ path: join(OUT, "time-2-keyframes.png"), clip: strip });

    // the diamonds render in chart order (every force section's points, each sorted by its own
    // coordinate) and this is the only force section — so a keyframe's diamond index is its
    // position among them, NOT the order it was appended in.
    const sOf = async (id: number): Promise<number | undefined> =>
        (await pts(1)).find((p) => p.id === id)?.s;
    const diaBox = async (id: number): Promise<{ x: number; y: number }> => {
        const i = (await pts(1)).findIndex((p) => p.id === id);
        const b = await page.locator(".fpt").nth(i).locator(".fhit").boundingBox();
        if (!b) throw new Error(`keyframe ${id}'s hit circle isn't laid out`);
        return { x: b.x + b.width / 2, y: b.y + b.height / 2 };
    };

    // ── 3. Grab one, move, and RETURN to the exact grab point: the excursion is the positive
    // control (it proves this rig detects a real write), and the return must land the stored t
    // back BIT-EXACTLY with no history entry. A drag that re-derived the coordinate absolutely
    // through the display inverse would drift it by the lens's O(step) gap on every grab. ──
    // Held under the Ctrl BYPASS on purpose: it zeroes the grid and the value landmarks, so
    // nothing can quantize a drift back onto the stored value and let this pass vacuously. What
    // survives the bypass is exactly the mechanism under test — the per-axis gesture-start
    // magnet — and the grab is deliberately off-centre on both axes, so an absolute
    // re-derivation would snap the point to the cursor instead of holding its own coordinate.
    const depth0 = await undoDepth();
    const c = await diaBox(held.id);
    const grab = { x: c.x + 3, y: c.y + 10 }; // inside FHIT_R, but past SNAP_PX on the g axis
    await page.keyboard.down("Control");
    await page.mouse.move(grab.x, grab.y);
    await page.mouse.down();
    await page.mouse.move(grab.x + 30, grab.y - 20, { steps: 6 });
    await expect.poll(() => sOf(held.id)).not.toBe(held.s); // positive control: it moves
    await page.mouse.move(grab.x, grab.y, { steps: 6 });
    await page.mouse.up();
    await page.keyboard.up("Control");
    expect(await sOf(held.id)).toBe(held.s); // bit-exact, not merely close
    expect((await pts(1)).find((p) => p.id === held.id)?.g).toBe(held.g); // the value axis too
    expect(await undoDepth()).toBe(depth0); // a zero-delta gesture records nothing

    // ── 4. A real drag on the OTHER keyframe writes in SECONDS and records one entry. ──
    const grabB = await diaBox(moved.id);
    await page.mouse.move(grabB.x, grabB.y);
    await page.mouse.down();
    await page.mouse.move(grabB.x + 40, grabB.y, { steps: 8 });
    await page.mouse.up();
    const dragged = await sOf(moved.id);
    if (dragged === undefined) throw new Error("the keyframe vanished under the drag");
    expect(dragged).toBeGreaterThan(moved.s);
    expect(dragged).toBeLessThan(dur + 1e-6); // still seconds, still inside the extent
    expect(await undoDepth()).toBe(depth0 + 1);

    // ── 5. The invariant-domain generalization: raise v₀ upstream. The cart covers more
    // ground per second, so this keyframe's DRAWN d slides — while its authored t, the axis
    // the integrator holds fixed, doesn't move at all. ──
    const beforeV0 = (await pts(1)).find((p) => p.id === held.id);
    if (!beforeV0) throw new Error("the keyframe vanished before the v0 edit");
    await page.evaluate(() => (window as any).__kex.setV0(18));
    await expect
        .poll(async () => (await pts(1)).find((p) => p.id === held.id)?.d)
        .not.toBe(beforeV0.d);
    const afterV0 = (await pts(1)).find((p) => p.id === held.id);
    if (!afterV0) throw new Error("the keyframe vanished after the v0 edit");
    expect(afterV0.s).toBe(beforeV0.s); // the authored coordinate held, bit-exact
    expect(afterV0.d).toBeGreaterThan(beforeV0.d); // faster entry ⇒ the same t reaches further
    await page.waitForTimeout(SHOT_MS);
    if (strip) await page.screenshot({ path: join(OUT, "time-2-retimed.png"), clip: strip });

    // ── 6. Flip the domain through the real section menu — a REMAP, not a reset: the
    // keyframes land on the distance axis where they were already being drawn. ──
    await page.locator(".clip").nth(1).click({ button: "right" });
    await expect(page.locator(".ctxmenu")).toBeVisible();
    const flip = page.locator(".ctxmenu").getByRole("menuitem", { name: "To distance" });
    await expect(flip).toBeEnabled();
    if (strip) await page.screenshot({ path: join(OUT, "time-3-menu.png"), clip: strip });
    await flip.click();
    await expect(page.locator(".ctxmenu")).toHaveCount(0);
    await expect.poll(async () => (await domains()).join(",")).toBe("0,0");
    const flipped = (await pts(1)).find((p) => p.id === held.id);
    if (!flipped) throw new Error("the keyframe vanished under the flip");
    // it now stores METRES — the distance it was already drawn at, so nothing moved on screen.
    expect(flipped.s).toBeGreaterThan(afterV0.s * 2); // seconds → metres, a different magnitude
    expect(flipped.d).toBeCloseTo(afterV0.d, 1); // and it draws where it already drew

    // undo restores the seconds payload byte-identically, one entry.
    await page.keyboard.press("Control+z");
    await expect.poll(async () => (await domains()).join(",")).toBe("0,1");
    expect((await pts(1)).find((p) => p.id === held.id)?.s).toBe(afterV0.s);

    // ── 7. MIXED-DOMAIN multi-drag: append a Distance force section, shift-click one keyframe from
    // each domain into one set, and drag. A marquee or shift-click spans sections freely, so the
    // shared delta cannot be a raw native number — it lives on the global d axis and each member
    // converts it through its OWN section's mapping. With the delta shared natively, the Time
    // member would take the anchor's METRES as seconds (a ~10× overshoot at these speeds). ──
    await page.locator(".clip-add").click();
    await page.getByRole("menuitem", { name: "Append force section" }).click();
    await expect.poll(async () => (await domains()).join(",")).toBe("0,1,0");
    await frameTimeline(page);
    // an INTERIOR keyframe in each section: a seed sits on a section bound (clamped against the
    // drag) and the boundary seeds of adjacent sections draw at the same point (an ambiguous click).
    const dcb = await page.locator(".clip").nth(2).boundingBox();
    const dbb = await page.locator(".dock .body").boundingBox();
    if (!dcb || !dbb) throw new Error("distance clip / timeline body not laid out");
    const seeded2 = await pts(2);
    await page.mouse.dblclick(dcb.x + dcb.width * 0.5, dbb.y + dbb.height * 0.5);
    await expect.poll(async () => (await pts(2)).length).toBe(seeded2.length + 1);
    const distPts = await pts(2);
    const distPt = distPts.find((q) => !seeded2.some((z) => z.id === q.id));
    if (!distPt) throw new Error("the distance keyframe wasn't found by id-diff");
    const timePt = (await pts(1))[1]; // an interior Time keyframe (seconds)
    // index the diamonds across BOTH force sections, in chart order
    const allPts = [...(await pts(1)), ...distPts];
    const diaAt = async (id: number): Promise<{ x: number; y: number }> => {
        const i = allPts.findIndex((q) => q.id === id);
        const b = await page.locator(".fpt").nth(i).locator(".fhit").boundingBox();
        if (!b) throw new Error(`keyframe ${id}'s hit circle isn't laid out`);
        return { x: b.x + b.width / 2, y: b.y + b.height / 2 };
    };
    const tBox = await diaAt(timePt.id);
    const dBox = await diaAt(distPt.id);
    await page.mouse.click(tBox.x, tBox.y);
    await page.keyboard.down("Shift");
    await page.mouse.click(dBox.x, dBox.y); // shift-click TOGGLES membership → a mixed set of two
    await page.keyboard.up("Shift");
    await expect
        .poll(() => page.evaluate((): number[] => (window as any).__kex.forceSelIds()))
        .toHaveLength(2);

    // grab the DISTANCE member as the anchor (its native frame is metres) and drag it right. Ctrl
    // bypasses the grid so the landing is the cursor's, not a quantum's.
    const depth1 = await undoDepth();
    await page.keyboard.down("Control");
    await page.mouse.move(dBox.x, dBox.y);
    await page.mouse.down();
    await page.mouse.move(dBox.x + 60, dBox.y, { steps: 10 });
    await page.mouse.up();
    await page.keyboard.up("Control");
    expect(await undoDepth()).toBe(depth1 + 1); // one entry for the whole block

    const distAfter = (await pts(2)).find((q) => q.id === distPt.id);
    const timeAfter = (await pts(1)).find((q) => q.id === timePt.id);
    if (!distAfter || !timeAfter) throw new Error("a member vanished under the mixed drag");
    const dMoved = distAfter.s - distPt.s; // metres of ride, the shared delta
    const tMoved = timeAfter.s - timePt.s; // seconds, the SAME ride through the time mapping
    expect(dMoved).toBeGreaterThan(1); // positive control: the anchor really moved
    expect(tMoved).toBeGreaterThan(0); // the co-selected member followed
    // the whole point: seconds, not metres. At these entry speeds (≥10 m/s) the same ride is at
    // most a tenth of the metres number — a raw shared native delta would make them equal.
    expect(tMoved).toBeLessThan(dMoved / 5);
    // both drawn d moved by the SAME shared amount — the rigid-offset law on the common axis.
    expect(timeAfter.d - timePt.d).toBeCloseTo(distAfter.d - distPt.d, 1);
    await page.waitForTimeout(SHOT_MS);
    if (strip) await page.screenshot({ path: join(OUT, "time-4-mixed.png"), clip: strip });

    // and a zero-delta grab-and-return on the mixed set is a byte-identical no-op for BOTH members
    // (each leg of every member's delta goes through its own mapping, so 0 maps to exactly 0).
    const depth2 = await undoDepth();
    const anchor2 = await diaAt(distPt.id);
    await page.keyboard.down("Control");
    await page.mouse.move(anchor2.x + 3, anchor2.y + 10);
    await page.mouse.down();
    await page.mouse.move(anchor2.x + 33, anchor2.y - 10, { steps: 6 });
    await expect
        .poll(async () => (await pts(1)).find((q) => q.id === timePt.id)?.s)
        .not.toBe(timeAfter.s); // positive control: the co-selected member moves mid-drag
    await page.mouse.move(anchor2.x + 3, anchor2.y + 10, { steps: 6 });
    await page.mouse.up();
    await page.keyboard.up("Control");
    expect((await pts(1)).find((q) => q.id === timePt.id)?.s).toBe(timeAfter.s);
    expect((await pts(2)).find((q) => q.id === distPt.id)?.s).toBe(distAfter.s);
    expect(await undoDepth()).toBe(depth2);
});
