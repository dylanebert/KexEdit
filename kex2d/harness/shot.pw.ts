...
test("kex2d timeline", async ({ page }) => {
    ...
    // author two force pins on the chart band, then re-shoot — exercises pins, the dashed
    // constraint curve, and the active pin's tangent handles. best-effort: a click miss must not
    // fail the capture, so the bare-track shots above are always produced first.
    try {
        const box = await page.locator(".dock").boundingBox();
        if (box) {
            // the chart band sits below the ruler+gap (~26px from the dock top); click safely inside
            await page.mouse.click(box.x + box.width * 0.38, box.y + 60);
            await page.mouse.click(box.x + box.width * 0.62, box.y + 45);
